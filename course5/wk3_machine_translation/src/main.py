...
def model_def(Tx, Ty, n_a, n_s, human_vocab_size, machine_vocab_size):
    """
    Arguments:
    ...
    model = Model(inputs=[X, s0, c0], outputs=outputs, name='DateRecognizer')
...
def main():
    ...
    model = model_def(Tx, Ty, n_a, n_s, len(human_vocab), len(machine_vocab))
    model.summary()
    ...
    # checkpoint = ModelCheckpoint(filepath="model.weights.best.hdf5", verbose=True)
    # model.fit([Xoh, s0, c0], outputs, epochs=20, batch_size=100, callbacks=[checkpoint], verbose=1)
